...
class TestFacebookRequest(PythonFacebookTestCase):
    ...
    def testGetParamsWillAutoAppendAccessTokenAndAppSecretProof(self):
        app = FacebookApp('123', 'foo_secret')
        request = FacebookRequest(app, 'foo_token', 'POST', '/foo',
                                  {'foo': 'bar'})
        params = request.get_params()
        self.assertEqual({
            'foo': 'bar',
            'access_token': 'foo_token',
            'appsecret_proof': 'df4256903ba4e23636cc142117aa632133d75c642bd2'
                               'a68955be1443bd14deb9',
        }, params)

    def testAnAccessTokenCanBeSetFromTheParams(self):
        app = FacebookApp('123', 'foo_secret')
        request = FacebookRequest(app, None, 'POST', '/me',
    ...
    def testAccessTokenConflictsWillThrow(self):
        app = FacebookApp('123', 'foo_secret')
        with self.assertRaises(FacebookSDKException):
            FacebookRequest(app, 'foo_token', 'POST', '/me',
                            {'access_token': 'bar_token'})

    def testAProperUrlWillBeGenerated(self):
        app = FacebookApp('123', 'foo_secret')
        getRequest = FacebookRequest(app, 'foo_token', 'GET', '/foo',
                                     {'foo': 'bar'})

        getUrl = getRequest.get_url()
        expectedParams = 'access_token=foo_token&appsecret_proof=df4256903ba' \
                         '4e23636cc142117aa632133d75c642bd2a68955be1443bd14d' \
                         'eb9&foo=bar'
        expectedUrl = '/{}/foo?{}'.format(Facebook.DEFAULT_GRAPH_VERSION,
                                          expectedParams)
        self.assertEqual(expectedUrl, getUrl)

        postRequest = FacebookRequest(app, 'foo_token', 'POST', '/bar',
                                      {'foo': 'bar'})
    ...
    def testAuthenticationParamsAreStrippedAndReapplied(self):
        ...
        request = FacebookRequest(
            ...
            params={'access_token': 'foo_token',
                    'appsecret_proof': 'bar_app_secret', 'bar': 'baz'})
        url = request.get_url()
        expectedParams = 'access_token=foo_token&appsecret_proof=df4256903b' \
                         'a4e23636cc142117aa632133d75c642bd2a68955be1443bd1' \
                         '4deb9&bar=baz'
        expectedUrl = '/{}/foo?{}'.format(Facebook.DEFAULT_GRAPH_VERSION,
                                          expectedParams)
        ...
        expectedParams = {
            'access_token': 'foo_token',
            'appsecret_proof': 'df4256903ba4e23636cc142117aa632133d75c642bd'
                               '2a68955be1443bd14deb9',
            'bar': 'baz',
        }
        ...
        #     }
        #     app = FacebookApp('123', 'foo_secret')
        #     request = FacebookRequest(app, 'foo_token', 'POST',
        #                               '/foo/photos', params)
