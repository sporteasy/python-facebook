...
class GraphEdgeTest(PythonFacebookTestCase):
    pagination = {
        'next': 'https://graph.facebook.com/v7.12/998899/photos?'
                'pretty=0&limit=25&after=foo_after_cursor',
        'previous': 'https://graph.facebook.com/v7.12/998899/photos?'
                    'pretty=0&limit=25&before=foo_before_cursor',
    }
    ...
    def testCanReturnGraphGeneratedPaginationEndpoints(self):
        graphEdge = GraphEdge(self.request,
                              metadata={'paging': self.pagination})
        nextPage = graphEdge.get_pagination_url('next')
        prevPage = graphEdge.get_pagination_url('previous')
        self.assertEqual(
            '/998899/photos?pretty=0&limit=25&after=foo_after_cursor',
            nextPage)
        self.assertEqual(
            '/998899/photos?pretty=0&limit=25&before=foo_before_cursor',
            prevPage)

    def testCanInstantiateNewPaginationRequest(self):
        graphEdge = GraphEdge(self.request,
                              metadata={'paging': self.pagination},
                              parent_edge_endpoint='/1234567890/likes')
        ...
        expected = '/v1337/998899/photos?access_token=foo_token&after=' \
                   'foo_after_cursor&appsecret_proof=857d5f035a894f16b41' \
                   '80f19966e055cdeab92d4d53017b13dccd6d43b6497af&foo=bar' \
                   '&limit=25&pretty=0'
        self.assertEqual(expected, nextPage.get_url())
        expected = '/v1337/998899/photos?access_token=foo_token&' \
                   'appsecret_proof=857d5f035a894f16b4180f19966e055cdeab92d' \
                   '4d53017b13dccd6d43b6497af&before=foo_before_cursor&' \
                   'foo=bar&limit=25&pretty=0'
    ...
    def testCanMapOverNodes(self):
        ...
        def _lambda(_node):
            _node.set_field('name',
                            _node.get_field('name').replace('dummy', 'foo'))
            return _node
