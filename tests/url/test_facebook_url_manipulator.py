...
def data_provider_dirty_urls():
    return (
        ...
        (
            ...
            'error_reason=abcd&error_description=abcd'
...
def data_provider_mergable_endpoints():
    return (
        (
...
class TestFacebookUrlManipulator(PythonFacebookTestCase):
    @data_provider(data_provider_dirty_urls)
    def test_params_get_removed_from_a_url(self, dirty_url,
                                           expected_clean_url):
        remove_params = [
            'state',
            'code',
        ...
        current_uri = FacebookUrlManipulator.remove_params_from_url(
            dirty_url, remove_params)
        self.assertEqual(expected_clean_url, current_uri)

    def test_gracefully_handles_url_appending(self):
        params = {}
        url = 'https://www.foo.com/'
        processed_url = FacebookUrlManipulator.append_params_to_url(url,
                                                                    params)
        ...
        processed_url = FacebookUrlManipulator.append_params_to_url(url,
                                                                    params)
        self.assertEqual('https://www.foo.com/?access_token=foo',
                         processed_url)
        ...
        processed_url = FacebookUrlManipulator.append_params_to_url(url,
                                                                    params)
        self.assertEqual('https://www.foo.com/?access_token=bar&foo=bar',
                         processed_url)
    ...
    def test_params_can_be_returned_as_array(self):
        ...
        paramsTwo = FacebookUrlManipulator.get_params_as_array(
            '/foo?one=1&two=2')
        paramsThree = FacebookUrlManipulator.get_params_as_array(
            'https://www.foo.com')
        paramsFour = FacebookUrlManipulator.get_params_as_array(
            'https://www.foo.com/?')
        paramsFive = FacebookUrlManipulator.get_params_as_array(
    ...
    @data_provider(data_provider_mergable_endpoints)
    def test_params_can_be_merged_onto_url_properly(self, url_one, url_two,
                                                    expected):
    ...
    def testGraphUrlsCanBeTrimmed(self):
        ...
        base_graph_url = FacebookUrlManipulator.base_graph_url_endpoint(
            full_graph_url)
