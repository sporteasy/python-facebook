...
from python_facebook.sdk.http_clients.http_clients_factory import \
    HttpClientsFactory
from python_facebook.sdk.persistent_data.persistent_data_factory import \
    PersistentDataFactory
from python_facebook.sdk.pseudo_random_string.\
    pseudo_random_string_generator_factory import \
    PseudoRandomStringGeneratorFactory as PSRSGF
...
class Facebook(object):
    ...
    def __init__(self, override_config=None):
        ...
        if not config['app_id']:
            raise FacebookSDKException(
                'Required "app_id" key not supplied in config and could '
                'not find fallback environment variable "{}"'.format(
                    self.APP_ID_ENV_NAME))

        if not config['app_secret']:
            raise FacebookSDKException(
                'Required "app_secret" key not supplied in config and could '
                'not find fallback environment variable "{}"'.format(
                    self.APP_SECRET_ENV_NAME))
        ...
        self.client = FacebookClient(
            HttpClientsFactory.create_http_client(
                config['http_client_handler']),
            config['enable_beta_mode']
        )
        self.prsg = PSRSGF.create_pseudo_random_string_generator(
            config['pseudo_random_string_generator']
        )
        self.set_url_detection_handler(
            config['url_detection_handler'] or FacebookUrlDetectionHandler())
        self.persistent_data_handler = PersistentDataFactory.\
            create_persistent_data_handler(config['persistent_data_handler'])
    ...
    def get_oauth2_client(self):
        if not isinstance(self.oauth2_client, OAuth2Client):
            self.oauth2_client = OAuth2Client(self.app, self.client,
    ...
    def set_default_access_token(self, access_token):
        if isinstance(access_token, str):
            ...
            return
        raise ValueError(
            'The default access token must be of type "string" or AccessToken')
    ...
    def post(self, endpoint, params=None, access_token=None, etag=None,
             graph_version=None):
        return self.send_request(
            'POST',
            endpoint,
    ...
    def delete(self, endpoint, params=None, access_token=None, etag=None,
               graph_version=None):
        return self.send_request(
            'DELETE',
            endpoint,
    ...
    def send_request(self, method, endpoint, params=None, access_token=None,
                     etag=None, graph_version=None):
        access_token = access_token or self.default_access_token
        graph_version = graph_version or self.default_graph_version
        request = self.request(method, endpoint, params, access_token, etag,
                               graph_version)

        self.last_response = self.client.send_request(request)
        return self.last_response

    def send_batch_request(self, requests, access_token=None,
                           graph_version=None):
        # access_token = access_token or self.default_access_token
        # graph_version = graph_version or self.default_graph_version
    ...
    def request(self, method, endpoint, params=None, access_token=None,
                etag=None, graph_version=None):
        access_token = access_token or self.default_access_token
        graph_version = graph_version or self.default_graph_version
    ...
    def upload_video(self, target, path_to_file, metadata=None,
                     access_token=None, max_transfer_tries=5,
                     graph_version=None):
        raise NotImplementedError
