...
from python_facebook.sdk.exceptions.facebook_authentication_exception import \
    FacebookAuthenticationException
from python_facebook.sdk.exceptions.facebook_authorization_exception import \
    FacebookAuthorizationException
from python_facebook.sdk.exceptions.facebook_client_exception import \
    FacebookClientException
from python_facebook.sdk.exceptions.\
    facebook_resumable_upload_exception import \
    FacebookResumableUploadException
from python_facebook.sdk.exceptions.facebook_other_exception import \
    FacebookOtherException
from python_facebook.sdk.exceptions.facebook_server_exception import \
    FacebookServerException
from python_facebook.sdk.exceptions.facebook_throttle_exception import \
...
VIDEO_UPLOAD_RESUMABLE_ERROR_CODES = (
    1363030, 1363019, 1363037, 1363033, 1363021, 1363041)
# Login status or token expired, revoked, or invalid
LOGIN_ERROR_CODES = (100, 102, 190)
...
class FacebookResponseException(FacebookSDKException):
    ...
    def create(self, response):
        ...
        if not ('error' in data and 'code' in data['error']) \
                and 'code' in data:
            data = {
                'error': data
            }
        ...
        if data['error'].get('error_subcode'):
            if data['error']['error_subcode'] \
                ...
                return FacebookResponseException(
                    response, FacebookAuthenticationException(message, code))
            elif data['error']['error_subcode'] \
                    in VIDEO_UPLOAD_RESUMABLE_ERROR_CODES:
                return FacebookResponseException(
                    response, FacebookResumableUploadException(message, code))

        if code in LOGIN_ERROR_CODES:
            return FacebookResponseException(response,
                                             FacebookAuthenticationException(
                                                 message, code))
        elif code in SERVER_ERROR_CODES:
            return FacebookResponseException(response,
                                             FacebookServerException(message,
                                                                     code))
        elif code in API_THROTTLING_ERROR_CODES:
            return FacebookResponseException(response,
                                             FacebookThrottleException(message,
                                                                       code))
        elif code in DUPLICATE_POST_ERROR_CODES:
            return FacebookResponseException(response,
                                             FacebookClientException(message,
                                                                     code))
        ...
        # All others
        return FacebookResponseException(response,
                                         FacebookOtherException(message, code))

    def __get(self, key, default=None):
        if 'error' in self.response_data \
                and key in self.response_data['error']:
