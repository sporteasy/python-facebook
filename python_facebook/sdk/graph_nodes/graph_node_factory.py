...
class GraphNodeFactory(object):
    ...
    def make_graph_node(self, subclass_name=None):
        ...
        return self.cast_as_graph_node_or_graph_edge(self.decoded_body,
                                                     subclass_name)

    def make_graph_achievement(self):
        return self.make_graph_node(
            self.BASE_GRAPH_OBJECT_PREFIX + '.' + 'GraphAchievement')

    def make_graph_album(self):
        return self.make_graph_node(
            self.BASE_GRAPH_OBJECT_PREFIX + '.' + 'GraphAlbum')

    def make_graph_page(self):
        return self.make_graph_node(
            self.BASE_GRAPH_OBJECT_PREFIX + '.' + 'GraphPage')

    def make_graph_session_info(self):
        return self.make_graph_node(
            self.BASE_GRAPH_OBJECT_PREFIX + '.' + 'GraphSessionInfo')

    def make_graph_user(self):
        return self.make_graph_node(
            self.BASE_GRAPH_OBJECT_PREFIX + '.' + 'GraphUser')

    def make_graph_event(self):
        return self.make_graph_node(
            self.BASE_GRAPH_OBJECT_PREFIX + '.' + 'GraphEvent')

    def make_graph_group(self):
        return self.make_graph_node(
            self.BASE_GRAPH_OBJECT_PREFIX + '.' + 'GraphGroup')
    ...
    def validate_response_as_dict(self):
        if not isinstance(self.decoded_body, dict):
            raise FacebookSDKException(
                'Unable to get response from Graph as dict.', 620)

    def validate_response_castable_as_graph_node(self):
        if 'data' in self.decoded_body and self.is_castable_as_graph_edge(
                self.decoded_body['data']):
            raise FacebookSDKException(
                'Unable to convert response from Graph to a GraphNode because '
                'the response looks like a GraphEdge. '
                'Try using GraphNodeFactory.make_graph_edge() instead.',
                620
            )

    def validate_response_castable_as_graph_edge(self):
        if not ('data' in self.decoded_body and self.is_castable_as_graph_edge(
                self.decoded_body['data'])):
            raise FacebookSDKException(
    ...
    def safely_make_graph_node(self, data, subclass_name=None):
        ...
        for key, val in data.items():
            ...
            if isinstance(val, dict):
                ...
                items[key] = self.cast_as_graph_node_or_graph_edge(
                    val, object_subClass, key, parent_node_id)
            else:
                items[key] = val

        return subclass(items)

    def cast_as_graph_node_or_graph_edge(self, data, subclass_name=None,
                                         parent_key=None, parent_node_id=None):
        """
        ...
        if 'data' in data:
            ...
            if self.is_castable_as_graph_edge(data['data']):
                return self.safely_make_graph_edge(data, subclass_name,
    ...
    def safely_make_graph_edge(self, data, subclass_name=None, parent_key=None,
                               parent_node_id=None):

        if not data.get('data'):
            raise FacebookSDKException(
                'Cannot cast data to GraphEdge. Expected a "data" key.', 620)

        data_list = []

        for graph_node in data['data']:
            data_list.append(
                self.safely_make_graph_node(graph_node, subclass_name))

        metadata = self.get_metadata(data)

        # We'll need to make an edge endpoint for this in case it's a
        # GraphEdge (for cursor pagination)
        parent_graph_edge_endpoint = '/{}/{}'.format(
            parent_node_id,
            parent_key) if parent_node_id and parent_key else None

        classname = self.BASE_GRAPH_EDGE_CLASS
        klass = get_class(classname)

        return klass(self.response.get_request(), data_list, metadata,
                     parent_graph_edge_endpoint, subclass_name)
    ...
    def validate_subclass(self, subclass):
        ...
        raise FacebookSDKException(
            'The given subclass "{}" is not valid. Cannot cast to an object '
            'that is not a GraphNode subclass.'.format(subclass),
            620
        )
