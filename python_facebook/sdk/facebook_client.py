...
class FacebookClient(object):
    ...
    def __init__(self, http_client_handler=None, enable_beta=False):
        self.http_client_handler = http_client_handler or \
            self.detect_http_client_handler()
    ...
    def get_base_graph_url(self, post_to_video_url=False):
        if post_to_video_url:
            return self.BASE_GRAPH_VIDEO_URL_BETA if self._enable_beta_mode \
                else self.BASE_GRAPH_VIDEO_URL

        return self.BASE_GRAPH_URL_BETA if self._enable_beta_mode else \
            self.BASE_GRAPH_URL
    ...
    def prepare_request_message(self, request):
        ...
        return (url, request.get_method(), request.get_headers(),
    ...
    def send_request(self, request):
        ...
        # Since file uploads can take a while, we need to give more time for
        # uploads
        ...
        raw_response = self.http_client_handler.send(url, method, body,
                                                     headers, timeout)
