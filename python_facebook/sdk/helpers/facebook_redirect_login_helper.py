...
class FacebookRedirectLoginHelper(object):
    ...
    def __init__(self, oauth2_client, persistent_data_handler=None, url_handler=None, prsg=None):
        self.oauth2_client = oauth2_client
        self.persistent_data_handler = persistent_data_handler or FacebookMemoryPersistentDataHandler()
        self.url_detection_handler = url_handler or FacebookUrlDetectionHandler()
        self.pseudo_random_string_generator = PseudoRandomStringGeneratorFactory.create_pseudo_random_string_generator(prsg)
    ...
    def make_url(self, redirect_url, scope, params=None, separator='&'):
        """
        Stores CSRF state and returns a URL to which the user should be sent to in order to continue the login
        process with Facebook.
        """
        state = self.persistent_data_handler.get('state')
        if not state:
            state = self.pseudo_random_string_generator.get_pseudo_random_string(self.CSRF_LENGTH)

        self.persistent_data_handler.set('state', state)

        return self.oauth2_client.get_authorization_url(redirect_url, state, scope, params, separator)
    ...
    def get_logout_url(self, access_token, next, separator='&'):
        ...
        if access_token.is_app_access_token():
            raise FacebookSDKException('Cannot generate a logout URL with an app access token.', 722)
        ...
        return 'https://www.facebook.com/logout.php?' + urllib.urlencode(sorted(params.items()))
    ...
    def get_re_authentication_url(self, redirect_url, scope=None, separator='&'):
        """
    ...
    def get_access_token(self, redirect_url=None):
        ...
        redirect_url = FacebookUrlManipulator.remove_params_from_url(redirect_url, ['state'])
